...
) : ControllerBase
{
    ...
    [HttpGet]
    [ProducesResponseType(typeof(IReadOnlyList<TaxeDto>), 200)]
    ...
    [HttpPost]
    [ProducesResponseType(typeof(TaxeDto), 201)]
    public async Task<IActionResult> Creer([FromBody] CreerTaxeRequest req, CancellationToken ct)
    {
        var guard = EntrepriseRequise(out var eId);
        if (guard is not null) return guard;
    ...
    [HttpPut("{id:guid}")]
    [ProducesResponseType(typeof(TaxeDto), 200)]
    public async Task<IActionResult> MettreAJour(
        Guid id, [FromBody] MettreAJourTaxeRequest req, CancellationToken ct)
    {
        var guard = EntrepriseRequise(out var eId);
        if (guard is not null) return guard;
